...
def get_prediction(sent):
    terms = ['you', 'we', 'us', 'he', 'she', 'her', 'him', 'You', 'We', 'Us', 'He', 'She', 'Her', 'I', 'i', 'it', 'its',
             'It', 'Its']
                    ...
                    if _noun_chunk.root.dep_ == 'nsubj' or _noun_chunk.root.dep_ == 'dobj':
                        for _pattern in pattern_list:
                            if re.findall(_pattern.lower(), _noun_chunk.text.lower()):
                                ...
                                final.append((str(_parsed_sentence), target, text_id, _noun_chunk.root.dep, str(_verb),
                                              _pattern.strip('\\b'), _noun_chunk.text.lower()))
...
def main():
    ...
    parser.add_argument("--output_file", default='',
                        help="Location to store output of AnthroScores for every text, optional")
    parser.add_argument("--output_sentence_file", default='',
                        help="Location to store output of parsed sentences with AnthroScores, optional")
    parser.add_argument("--text_id_name", type=str, default='',
                        help="ID for each text, optional -- otherwise defaults to the index in the dataframe")
